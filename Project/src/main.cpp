...
int main() {
  ...
  int lane = 1;
  double max_speed = 49.5*0.45;     // 49.5 MPH limit converted to [m/s] 
  double speed_limit = 50*0.45; //  50 MPH speed limit converted [m/s]
  double safe_distance = 50;// meters. Minimum front and back distance for car to to take decisions.
  double ref_velocity = 0.0;      // [m/s]. End point speed in trajectory vector. Zero since it starts from rest.

  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
               &map_waypoints_dx,&map_waypoints_dy,
               &max_speed,&lane,&speed_limit,&safe_distance,&ref_velocity,&max_s]
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double speed_in_front;
          double distance_to_car =  safe_distance +1; // greater (farther) than safe_distance
          ...
          { 
            ...
            { 
              if ( check_car_s>car_s && check_distance<safe_distance )
              {
                ...
                speed_in_front = check_speed;
                distance_to_car = check_distance; 
              }
            }
          ...
          */
          ...
          { 
            if ( ref_velocity > target_speed || distance_to_car < safe_distance/2 )
            {
              ref_velocity -= 0.1;  //[m/s] equivalent to acceleration of -5 [m/s^2]
            }
            else if ( ref_velocity < target_speed )
            {
              ref_velocity += 0.1;  //[m/s] equivalent to acceleration of 5 [m/s^2]
            }
            ref_velocity = std::max(0.00001,ref_velocity);   // prevents from going backwards
            // N is the number of points along our spline
            double N = (target_dist/(0.02*ref_velocity)); // spacing between points on line so that car travels at ref_velocity as simulator has no controller
