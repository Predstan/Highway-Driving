...
double HBF::heuristic(double x, double y, double theta, vector<int> &goal){
  
  double cost = fabs(y - goal[0]) + fabs(x - goal[1]); //return grid distance to goal
  if (theta!=goal[2]){
    cost+=5;
  }
  return cost;
...
HBF::maze_path HBF::search(vector< vector<int> > &grid, vector<double> &start, 
  ...
  state.x = start[0];
  state.y = start[1];
  state.f = g + heuristic(state.x, state.y, theta, goal);
  state.theta = theta;
